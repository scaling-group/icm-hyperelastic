...
class SaveStress(L.Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs: dict, batch: PyTree, batch_idx, dataloader_idx=0):
        """Cache valid batch outputs. Save true and predicted stress to plot results."""
        ...
        self._save_stress(valid_dirpath, batch, outputs, trainer.global_rank, batch_idx)
    ...
    def on_test_batch_end(self, trainer, pl_module, outputs: dict, batch: PyTree, batch_idx, dataloader_idx=0):
        ...
        self._save_stress(test_dirpath, batch, outputs, trainer.global_rank, batch_idx)

    def _save_stress(self, dirpath: Path, batch: PyTree, outputs: dict, rank: int, batch_idx: int) -> None:
        ...
        # save preds
        for key, tensor in ptu.to_numpy(outputs["preds"]).items():
            if "calibrated_stress_Ss" in key or "scale" in key:
